...
var SheetUtils = (function (ns) { 
  ...
  */
  ...
  /**
  * turn objectified data(headless) into sheet writable data 
  * @param {object} a dataObject of objectified sheet data
  * @return {[]} a data array in sheet format
  */
  ns.datifyHeadlessObject = function(dataObject) {
    return [_.map(_.toArray(dataObject).slice(0,-1), function(value) {
      return value;
    })];
  };
  ...
  */
  ...
  /**
  * return headers as an array in given sheet.
  * @param {Sheet} currentSheet sheet which wants to get headers
  * @return [] an array for header strings
  **/
  ns.getSheetHeader = function(currentSheet) { 
    var headerRange = currentSheet.getDataRange().offset(0, 0, 1);
    return headerRange.getValues()[0];
  }
  
  /**
  * return values of specific row index
  * @param {Sheet} currentSheet sheet which wants to get row datas
  * 
  **/
  ns.datifyRow = function(currentSheet, editedRowIndex) {
    var editedRowRange = currentSheet.getDataRange().offset(editedRowIndex - 1, 0, 1);
    
    return editedRowRange.getValues();
  }
  ...
  * @return  {[object]} the objectified data
  **/
  ns.objectifyRow = function(currentSheet, editedRowIndex) {
    var headers = this.getSheetHeader(currentSheet);
    var retObj = this.objectifyData(SheetUtils.indexifyHeaders(headers), this.datifyRow(currentSheet, editedRowIndex));    
    retObj[0].range = currentSheet.getRange(editedRowIndex, 1, 1, headers.length);
    return retObj[0];
  }
  
  /**
  * return specific range for given property
  * @param {object} rowObj the objectified data
  * @param {object} sheetConst Constant object for specific sheet
  * @param string property property in rowObj
  * @return {range}
  **/
  ns.getRangeForProperty = function(rowObj, sheetConst, property) {
    return rowObj.range.getCell(1, sheetConst.headerIndexes[property] + 1);
  }
...
function testGetObjectifyRow() {  
  Logger.log(SheetUtils.objectifyRow(CSSheet.sheet, 4));
  }
